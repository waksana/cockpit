...
| 扩展位置 | 能做什么 | 如何接入 |
| --- | --- | --- |
| 前端 | 增强输入区、消息、会话状态和导航；扩展草稿数据、链接与图片的呈现。 | 共用本体 React 和主题，通过 state 服务、组件 middleware、草稿 schema 与 Markdown 渲染注册接入。 |
| 后端 | 提供模块 HTTP 接口、观察原生事件，管理自己的配置和业务数据。 | 在本体 Node 进程中加载，使用模块独立的路由命名空间和数据目录，沿用同一个服务端口。 |
...
当前采用**可信本地包、冷加载**：安装、启用或停用在下次启动时生效，不会热改正在运行的服务。
