...
上述截图为经典界面。**经典 UI（`/`）是主要界面，也是 UI 请求的默认修改目标。**
0.3.0 源码新增的独立 **`/next/` 新版入口仅为实验界面**，仍可访问，
采用 shadcn 组件和会话优先布局；可直接返回经典入口。
未明确要求实验界面时，不随经典 UI 改动同步适配；具体边界见
[前端规范](docs/frontend-guidelines.md#classic-primary-ui)。
两套界面使用同一后端和持久化数据，切换不是撤销，也不会转移进行中的上传、录音或请求。
新版配套 File 0.2.0 与 Speech 0.9.0；Task、Notification 的界面仍在经典版使用。
源码能力与已发布、已部署版本不同，配套范围见[发行说明](docs/release-notes.md)。
