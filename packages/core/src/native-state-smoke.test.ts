...
test('native state: isolated public reads, no cached metadata and explicit unloaded values', {
  ...
  const root = mkdtempSync(join(tmpdir(), 'cockpit-state-native-'));
  ...
  const provider = createServer(async (req, res) => {
    ...
    if (request.stream) {
      ...
      for (const choice of [{ delta: { role: 'assistant', content: answer }, finish_reason: null }, { delta: {}, finish_reason: 'stop' }]) {
