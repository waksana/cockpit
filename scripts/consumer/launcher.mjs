...
import { ConsumerModuleRunner } from './module-runner.mjs';
...
export class ConsumerLauncher {
  ...
  checkCompatibility(selection) {
    if (selection.moduleRunnerApi !== 1) throw new Error('Main selection must support resident module runner API1');
  ...
  async startChild(selection, operation) {
    ...
    this.receipt(operation);
    await this.moduleRunner.ensure(selection, operation.operationId);
    if (operation.kind === 'install' && operation.recovery !== 'fallback') this.verifyInstallAuthorization(operation);
    ...
    try {
      processHandle = spawn(process.execPath, ['--import', 'tsx', 'src/index.ts'], {
        cwd: join(selection.release, 'apps/server'), detached: true, stdio: ['ignore', log, log],
    ...
    let resolveExit;
    const child = { handle: processHandle, identity, exited: new Promise(resolve => { resolveExit = resolve; }) };
    this.child = child;
    ...
    while (this.child === child && Date.now() < deadline) {
      try {
        ...
        writeJson(join(this.root, 'runtime.json'), { state: 'healthy', identity, process: child.process,
          startupVerifiedAt: new Date().toISOString() });
        writeJson(join(this.root, 'known-good.json'), { ...selection, identity });
        operation.observed = identity;
  ...
  async drain(operation, requireHealthy = true) {
    ...
    this.receipt(operation);
    const acknowledgement = await boundedJson(await fetch(this.endpoint('/admin/restart'), {
      method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ pending: true }),
      redirect: 'error', signal: AbortSignal.timeout(10_000),
    }));
    if (acknowledgement.restartPending !== true) throw new Error('Backend did not acknowledge native safe-idle drain');
    operation.drainAcknowledged = true;
    this.receipt(operation);
  ...
  acceptStop(body) {
    ...
    this.claim(operation);
    return this.run(operation, async () => {
      await this.moduleRunner.assertIdle();
      operation.state = 'stopping-module-runner';
      // Runner IPC is a lifecycle mutation even before backend drain.
      operation.backendEffect = true;
      this.receipt(operation);
      try { await this.moduleRunner.shutdownIfIdle(operation.operationId); }
      catch (error) {
        if (error.runnerIdleRefused) {
          operation.error = `${error.message}; launcher and runner retained`;
          this.complete(operation, 'failed');
          return;
        }
        throw error;
      }
  ...
  acceptRestart(body) {
    ...
    return this.run(operation, async () => {
      ...
      this.receipt(operation);
      await this.drain(operation);
      if (!operation.oldExited || !operation.drainAcknowledged || operation.drainRequested !== child.identity.instanceId
  ...
  recover(body) {
    ...
    if (!['verify', 'drain', 'fallback', 'abandon'].includes(body.recovery)) throw new Error('Recovery is verify, drain, fallback or abandon');
    return this.run(operation, async () => {
      if (body.recovery === 'verify') {
        ...
        writeJson(join(this.root, 'runtime.json'), { state: 'healthy', identity: operation.identity,
          process: child.process, startupVerifiedAt: new Date().toISOString() });
        writeJson(join(this.root, 'known-good.json'), { ...operation.runningSelection, identity: operation.identity });
        operation.observed = operation.identity;
        this.complete(operation, operation.recovery === 'fallback' ? 'recovered' : 'succeeded');
        ...
        await this.drain(recovery, false);
        operation.state = 'unknown';
