...
> **历史归档，不是当前规范或操作指南。** [当前文档](../README.md) · [归档边界](README.md) · [原位置固定版本](https://github.com/waksana/cockpit/blob/a1f4a9a7c9e72b151958c270c4f790b32b72636e/docs/chat-design-review.md)。
> 结论、行号、旧路径和环境按原记录理解，未为本次文档整理重新执行其操作。


> 历史记录。下述附件/语音场景、文件组件和旧静态 review 工具已随增强停放，
> 不再是当前 lab 或产品的可用能力。原生文字、工具、决策和滚动场景保留；
> 当前职责见[模块目录](../module-catalog.md)。
...
后续 v4 已收回输入区悬浮外框和没有实机必要性证据的全局 VisualViewport 控制器，
恢复 CSS 满宽底栏、内部安全区与简短占位，保留用户时间戳外置。
本页是初轮全组件记录；后续 [CSS 优先的定向返工说明](chat-composer-rework.md)
保留其新旧对照和设备证据边界。薄本体再按模块目录摘除非原生增强。
