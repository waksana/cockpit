...
`frontend` 只接受 `entry`、`styles`、`assets` 与 `worker`；manifest 各层均严格校验，
含其他字段的包会被拒绝安装或加载。
...
| 展示能力 | `uiVersion: 1` / `uiSurfaceVersion: 1` | 分别检查；只声明公共 CSS 能力 |
...
激活 context 另提供 `context.uiVersion: 1`，声明已实现的公共语义 CSS 与图标规范。
