...
| sessionStatus | 原生并存活动摘要/错误/真实待决策和非交互 children，不嵌套按钮；activity、loaded、connected 提供采样及可用边界，缺失不代表 idle；activityRefreshing 仅表示浏览器 control 读取正在进行；activityDisplay 可保留上次采样外观或表达读取错误，只用于展示，绝不授权操作 |
