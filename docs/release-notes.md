...
- **Breaking configuration change:** `COCKPIT_HOME` is now the host root
  (default `~/.cockpit`), with native Copilot state in its `copilot/` child.
  No old-home fallback or automatic migration is performed. Authenticate and
  start using the same selected root; existing installations require an explicit data decision.
