...
本文维护后续模块的用户能力和协作边界。当前尚无模块加载器，以下能力需要按
[模块协议](module-contract-draft.md)实现接入；表中的显示名称不决定 moduleId。

## 能力

| 模块能力 | 用户能力 | 与本体的协作 |
| --- | --- | --- |
| 文件 | 上传、托管、文件库、下载、输入区文件交互和聊天媒体展示。 | 负责字节传输、元数据和引用解析，提供 SDK 原生附件；使用本体的草稿及事件窗口。 |
