...
目标退出入口应作为同一可发现的公开 API 提供，使用现有认证和明确的受理/等待结果；
具体名字和 schema 尚未实现，不以现行私有部署分支冒充新接口。
...
Copilot 登录仍归原生运行时。当前外部 launcher 仍是实际启动链，
“外部进程”不代表代码已迁出；其当前事实见[架构边界](cockpit-plan.md#launchers)。
目标不再要求本体理解或依赖这些 launcher，迁移不得未经授权破坏现有运行方式。
...
现行尚未迁出的流程见[交付指南](DELIVERY.md)，不再是未来本体的必要依赖。
