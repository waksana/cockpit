...
| 首版冷加载 | 当前基线不做热插拔或运行中代码替换。安装、代码更新、停用/移除的变更记录为待重启生效，当前实际加载版本不变；下次本体启动加载选定版本。不是永久排除后续受控热生命周期。 |
| 后续受控模块生命周期 | [#5](https://github.com/waksana/cockpit/issues/5) 已确认模块管理及受控热启用/停用/更新目标；尚未实现，不属于菜单注册变更，也不改变当前冷加载操作要求。 |
...
| 已确认 | 当前为冷加载、前后端同包、可信主进程 import；#5 的受控热启用/停用/更新及独立 MCP path 仍是后续目标。 |
