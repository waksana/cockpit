...
<a id="classic-primary-ui"></a>
**经典 UI（根路径 `/`）是主要界面；独立的 `/next/` 新版 UI 仅为实验界面。**
用户未明确点名 `/next/` 时，UI 功能、样式、交互和修复请求都默认针对经典 UI。
普通请求不必反复确认经典还是新版，也不能为了双端一致擅自扩展范围。
实验界面的改动或适配需要用户明确纳入范围；经典 UI 的改动不要求同步到实验界面，
也没有一律追加实验界面适配或重复双端 UI 验收的要求。

两套界面仍共用原生业务能力与数据，不复制后端或建立第二份状态权威。
经典优先不豁免共享后端、状态和公开契约的正确性：改动共享层仍须按实际影响检查，
不能破坏共享数据或掩盖 API 错误；这不等于承诺两套 UI 功能对等。
`/next/` 入口仍存在，本文不移除或禁用它；[删除计划 #136](https://github.com/waksana/cockpit/issues/136)
已暂停，恢复实施需要用户另行明确授权。

以下新版实现约定仅在明确涉及实验界面的工作中适用，不构成同步改动要求。
实验新版使用独立入口与 shadcn 设计语言，不要求复现经典界面的 DOM、
组件拆分、颜色或布局细节。新版的组件及样式不得进入经典入口的依赖链。
