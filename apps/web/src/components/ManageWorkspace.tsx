...
type ModuleSkillTarget = { moduleId: string; resourceId: string };

function Provenance({ modules }: { modules?: ModuleSource[] }) {
  return modules?.map(module => <ModuleSourceBadge key={module.id} module={module} />);
}

function NavRow({ section, name, summary, modules, selected, detailOpen = false, enabled, disabled, onChange }: {
  section: ManageSection; name: string; summary?: string; modules?: ModuleSource[]; selected: string | null;
  detailOpen?: boolean; enabled?: boolean; disabled: boolean; onChange: GlobalToggle;
}) {
  ...
  return <ResourceRow name={name} badge={<Provenance modules={modules} />}
    link={{ to: `/${section}/${encodeURIComponent(name)}`, replace: detailOpen || selected !== null, selected: selected === name }}
...
function SkillsList({ selected, selectedModule, catalog, modules, onChange }: {
  selected: string | null; selectedModule: ModuleSkillTarget | null;
  catalog: Catalog<SkillGlobal>; modules: Catalog<ModuleRoleResources>; onChange: GlobalToggle;
}) {
  const { data: rows, status, failed, pending, valid } = catalog;
  const detailOpen = selected !== null || selectedModule !== null;
  const group = <ModuleProvidedGroup section="skills" catalog={modules} native={rows}
    selected={selectedModule} detailOpen={detailOpen} />;
...
type ModuleSkillRead = Awaited<ReturnType<CockpitApi['roleSkillRead']>>;
type SkillResource = Pick<ReturnType<typeof useKeyedResource<SkillRead | ModuleSkillRead>>,
  'data' | 'status' | 'failed' | 'pending' | 'errorCause'>;

export function SkillDetailContent({ resource, moduleSkill = false }: { resource: SkillResource; moduleSkill?: boolean }) {
  const { data, status, failed, pending, errorCause } = resource;
  const notFound = <StateNotice kind="empty" placement="pane">
    {moduleSkill ? '未找到该模块 Skill，模块可能已停用或更新。' : '未找到该 Skill。'}
  </StateNotice>;
  // A structured not-found supersedes any retained earlier read.
  if (failed && isSkillNotFoundError(errorCause)) return notFound;
  // A failed module refresh never leaves an earlier version's body on screen.
  if (failed && moduleSkill) return <ResourceStatus status={status} failed pending={pending} placement="pane" />;
  if (!data) return status ? <ResourceStatus status={status} failed={failed} pending={pending} placement="pane" /> : notFound;
  const nativeMeta = 'source' in data
    ? [skillSourceLabel(data.source), data.userInvocable ? '可手动调用' : null].filter(Boolean).join(' · ')
    : '';
  const module = 'module' in data ? data.module : undefined;
  const body = data.body === undefined ? '' : skillBodyContent(data.body);
  return <PaneBody className="manage-detail">
    <div className="manage-detail-column">
      <ResourceStatus status={status} failed={failed} pending={pending} />
      {module ? <div className="manage-detail-meta"><ModuleSourceBadge module={module} /></div>
        : nativeMeta && <div className="manage-detail-meta">{nativeMeta}</div>}
...
// Routes: /mcp, /skills, native /:item details and version-bound module Skill details.
export function ManageWorkspace({ section: selectedSection }: { section?: ManageSection }) {
  const { pathname } = useLocation();
  const { item = null, moduleId, resourceId } = useParams();
  const segment = pathname.split('/')[1];
  const section: ManageSection = selectedSection ?? (segment === 'skills' ? segment : 'mcp');
  const moduleSkill = section === 'skills' && moduleId && resourceId ? { moduleId, resourceId } : null;
  return <ManagementContent key={section} section={section} item={item} moduleSkill={moduleSkill} />;
}

function ManagementContent({ section, item, moduleSkill }: {
  section: ManageSection; item: string | null; moduleSkill: ModuleSkillTarget | null;
}) {
  ...
  const moduleId = moduleSkill?.moduleId;
  const resourceId = moduleSkill?.resourceId;
  const loadModuleSkill = useCallback(() => moduleId && resourceId
    ? cockpitApi.roleSkillRead(moduleId, resourceId)
    : Promise.reject(new Error('No module Skill selected')), [moduleId, resourceId]);
  const moduleSkillResource = useKeyedResource(
    moduleSkill ? `global:module-skill:${moduleSkill.moduleId}:${moduleSkill.resourceId}` : 'global:module-skill:none',
    loadModuleSkill, refreshNonce, moduleSkill !== null,
  );
  const catalog = section === 'mcp' ? mcpCatalog : skillCatalog;
  const nativeModules = catalog.data?.find(row => row.name === item)?.modules;
  const selectedModuleRow = moduleSkill && moduleProvidedRows(moduleCatalog.data ?? [], 'skills', skillCatalog.data)
    .find(row => row.module.id === moduleSkill.moduleId && row.resourceId === moduleSkill.resourceId);
  const moduleSource = moduleSkillResource.data?.module ?? selectedModuleRow?.module;
  const displayedItem = moduleSkill ? moduleSkillResource.data?.name ?? selectedModuleRow?.name ?? '模块 Skill' : item;
  return <ManagementShell section={section} item={displayedItem} onRefresh={refresh}
    titlePrefix={moduleSkill ? <Provenance modules={moduleSource ? [moduleSource] : undefined} />
      : catalog.usable && <Provenance modules={nativeModules} />}
    master={section === 'mcp' ? <McpList catalog={mcpCatalog} modules={moduleCatalog} selected={item} onChange={onChange} />
      : <SkillsList catalog={skillCatalog} modules={moduleCatalog} selected={moduleSkill ? null : item}
        selectedModule={moduleSkill} onChange={onChange} />}
    detail={displayedItem === null ? <StateNotice kind="empty" placement="pane" className="manage-selection-hint">选择左侧的一项查看详情。</StateNotice>
      : section === 'mcp' ? <McpDetail catalog={mcpCatalog} name={item!} />
        : moduleSkill ? <SkillDetailContent resource={moduleSkillResource} moduleSkill />
