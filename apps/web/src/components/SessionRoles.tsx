    ...
    <div className="info-section-content info-controls">
      ...
      {!connected ? <StateNotice>等待连接，应用状态未确认。</StateNotice>
        : !session.loaded ? <StateNotice>会话未加载；已保存的角色将在下次加载时应用。</StateNotice>
          : session.rolesNeedReload || resultNeedsReload ? <StateNotice>角色选择已保存，重新加载会话后生效。</StateNotice> : null}
      ...
      {needsInspection && !action.busy && (action.error && operation === 'save'
        ...
        : <OperationResult state="unknown" action={{ label: '刷新模块角色', onClick: refresh, disabled: !connected }}>
          {copy.unknown(result?.error ?? '')}
      ...
      {result && result.status !== 'uncertain'
        ...
        && <OperationResult state={result.error ? 'info' : 'done'}>
        {result.status === 'saved' ? '已保存角色选择，未重新加载会话。' : '角色选择未变。'}
        {result.error && ` ${result.error}`}
        {result.recovery && <div>{result.recovery}</div>}
      </OperationResult>}
      {refreshed && <OperationResult state="done">已刷新角色状态；应用不代表能力就绪。</OperationResult>}
