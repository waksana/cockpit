...
export function CopyButton({ text, label = '复制', textOnly = false }: { text: string; label?: string; textOnly?: boolean }) {
  ...
  return <span className={`chat-copy${textOnly ? ' is-text' : ''}`}>
    <button type="button" className="chat-copy-button" aria-label={label} title={label}
      ...
      {!textOnly && <Icon name={state === 'copied' ? 'check' : 'file'} size={14} />}
