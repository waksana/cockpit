...
export function AskCard({ request, pending, onChoice }: {
  ...
  return <PendingDecision label="需要你的选择" title="需要你的回答" icon={<Icon name="newchat" size={16} />}
    pending={pending} hint={pending ? '正在提交回答…' : request.allowFreeform === false ? '请选择一个选项。' : '也可以在下方输入自己的回答。'}>
...
export function ElicitationCard({ request, pending, onSelect }: {
  ...
  return <PendingDecision label="需要你的输入" title="工具请求确认" icon={<Icon name="mcp" size={16} />}
    pending={pending} hint={pending ? '正在提交选择…' : undefined}>
