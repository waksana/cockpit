...
test('design scenarios cover short authoritative summaries, missing config and real native failures without a backend', async t => {
  ...
  const global = await cockpitApi.mcpGlobal();
...
test('resource scene exercises actual names and explicit provenance without HTTP', async t => {
  ...
  const directory = await cockpitApi.listDir();
  ...
  const added = await cockpitApi.addRoles(workspaceSessionId, [roles[2]]);
  ...
  assert.equal((await cockpitApi.roleReadiness(workspaceSessionId)).ready, false);
  ...
  assert.equal((await cockpitApi.addRoles(workspaceSessionId, [roles[2]])).status, 'unchanged');
...
test('resource role saving allows busy work and never loads an unloaded fixture', async t => {
  ...
  for (const loaded of [true, false]) {
    ...
    const state = store.getState();
    const roles = await cockpitApi.listRoles();
    const result = await cockpitApi.addRoles(workspaceSessionId, [roles[2]]);
    ...
    assert.equal((await cockpitApi.roleReadiness(workspaceSessionId)).rolesNeedReload, loaded);
...
test('global resource fixtures use native contracts, isolated mutations and explicit empty/error/loading states', async t => {
  ...
  installResourceFixture(store);
  const state = store.getState();
  const mcp = await cockpitApi.mcpGlobal();
  ...
  await cockpitApi.mcpSetDefault(mcp[0].name, false);
  assert.equal((await cockpitApi.mcpGlobal())[0].defaultOn, false);
  assert.equal((await state.mcpSession(workspaceSessionId))[0].enabled, true);
  const skills = await cockpitApi.skillsGlobal();
  Intents['skills/global'].result.parse({ skills });
  assert.equal(skills.find(skill => skill.name === 'unknown-default')?.enabled, undefined);
  await cockpitApi.skillsSetGlobal(skills[0].name, false);
  const skill = await cockpitApi.skillsRead(skills[0].name);
  ...
  await assert.rejects(cockpitApi.mcpGlobal(), /Synthetic resource failure/);
  ...
  let settled = false;
  const pending = cockpitApi.skillsGlobal().then(value => { settled = true; return value; });
  await Promise.resolve();
  assert.equal(settled, false);
  release();
  assert.ok((await pending).length > 0);
  installResourceFixture(store, false, { failMutations: true });
  assert.ok((await cockpitApi.mcpGlobal()).length > 0);
  await assert.rejects(cockpitApi.mcpSetDefault('cockpit-task', false), /Synthetic mutation failure/);
  assert.equal((await cockpitApi.mcpGlobal())[0].defaultOn, true);
  assert.equal(fetch.mock.callCount(), 0);
});
