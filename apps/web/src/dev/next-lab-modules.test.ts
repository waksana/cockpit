...
import { labModuleHandler, loadLabModules } from '../../next-lab-modules';

async function fixture(t: TestContext) {
  const root = await mkdtemp(join(tmpdir(), 'cockpit-module-lab-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  await mkdir(join(root, 'dist/web/next'), { recursive: true });
  ...
  const manifest = {
    ...
    frontend: {
      ...
      next: { entry: 'dist/web/next/index.js', styles: ['dist/web/next/styles.css'] },
    },
  };
  const content = new Map([
    ['cockpit.module.json', JSON.stringify(manifest)],
    ['dist/web/index.js', 'export const classic = true;'],
    ['dist/web/styles.css', '.classic-fixture { color: black; }'],
    ['dist/web/next/index.js', 'export const next = true;'],
    ['dist/web/next/styles.css', '.next-fixture { color: black; }'],
...
test('module lab serves only inventoried frontend assets from clean receipt-bound packages', async t => {
  ...
  assert.ok(loaded.assets.has(module.next!.entry));
  ...
  await writeFile(join(root, 'dist/web/next/index.js'), 'tampered');
  await assert.rejects(loadLabModules({ file: root }), /Unsafe|receipt/);
});

test('synthetic file requests cover actual upload shape, explicit retry, media and discard without a backend', async t => {
  ...
  const upload = () => fetch(`${api}/upload?name=fail-once.png&operationId=synthetic-operation`, {
  ...
  assert.match(result.attachment.path, /^\/synthetic\/next-lab\/files\/f_[a-f0-9]{64}\/ready\/body$/);
  ...
  assert.equal((await fetch(`${origin}${module.next!.entry}`)).headers.get('content-type'), 'text/javascript');
